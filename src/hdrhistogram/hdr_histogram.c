...
static int64_t counts_get_normalised(const struct hdr_histogram* h,
                                     int32_t index)
{
    return counts_get_direct(h, normalize_index(h, index));
}

static void counts_inc_normalised(struct hdr_histogram* h, int32_t index,
                                  int64_t value)
{
...
static void counts_inc_normalised_atomic(struct hdr_histogram* h, int32_t index,
                                         int64_t value)
{
    int32_t normalised_index = normalize_index(h, index);
...
static void update_min_max_atomic(struct hdr_histogram* h, int64_t value)
{
    ...
    } while (!hdr_atomic_compare_exchange_64(&h->min_value, &current_min_value,
                                             value));
    ...
    } while (!hdr_atomic_compare_exchange_64(&h->max_value, &current_max_value,
                                             value));
}
...
{
    int32_t pow2ceiling =
        64 - count_leading_zeros_64(
                 value |
                 h->sub_bucket_mask); /* smallest power of 2 containing value */
    return pow2ceiling - h->unit_magnitude -
...
static int32_t counts_index(const struct hdr_histogram* h, int32_t bucket_index,
                            int32_t sub_bucket_index)
{
    /* Calculate the index for the first entry in the bucket: */
    /* (The following is the equivalent of ((bucket_index + 1) *
     * subBucketHalfCount) ): */
    int32_t bucket_base_index = (bucket_index + 1)
                                << h->sub_bucket_half_count_magnitude;
    /* Calculate the offset in the bucket: */
    int32_t offset_in_bucket = sub_bucket_index - h->sub_bucket_half_count;
    /* The following is the equivalent of ((sub_bucket_index  -
     * subBucketHalfCount) + bucketBaseIndex; */
...
int32_t counts_index_for(const struct hdr_histogram* h, int64_t value)
{
    ...
    int32_t sub_bucket_index =
        get_sub_bucket_index(value, bucket_index, h->unit_magnitude);
...
int64_t hdr_size_of_equivalent_value_range(const struct hdr_histogram* h,
                                           int64_t value)
{
    int32_t bucket_index = get_bucket_index(h, value);
    int32_t sub_bucket_index =
        get_sub_bucket_index(value, bucket_index, h->unit_magnitude);
    int32_t adjusted_bucket = (sub_bucket_index >= h->sub_bucket_count)
                                  ? (bucket_index + 1)
                                  : bucket_index;
    return INT64_C(1) << (h->unit_magnitude + adjusted_bucket);
}

static int64_t lowest_equivalent_value(const struct hdr_histogram* h,
                                       int64_t value)
{
    int32_t bucket_index = get_bucket_index(h, value);
    int32_t sub_bucket_index =
...
int64_t hdr_next_non_equivalent_value(const struct hdr_histogram* h,
                                      int64_t value)
{
    return lowest_equivalent_value(h, value) +
           hdr_size_of_equivalent_value_range(h, value);
}

static int64_t highest_equivalent_value(const struct hdr_histogram* h,
                                        int64_t value)
{
    return hdr_next_non_equivalent_value(h, value) - 1;
}

int64_t hdr_median_equivalent_value(const struct hdr_histogram* h,
                                    int64_t value)
{
    return lowest_equivalent_value(h, value) +
           (hdr_size_of_equivalent_value_range(h, value) >> 1);
}
...
static int32_t buckets_needed_to_cover_value(int64_t value,
                                             int32_t sub_bucket_count,
                                             int32_t unit_magnitude)
{
    int64_t smallest_untrackable_value = ((int64_t)sub_bucket_count)
                                         << unit_magnitude;
...
int hdr_calculate_bucket_config(int64_t lowest_discernible_value,
                                ...
                                struct hdr_histogram_bucket_config* cfg)
{
    ...
    largest_value_with_single_unit_resolution =
        2 * power(10, significant_figures);
    sub_bucket_count_magnitude = (int32_t)ceil(
        log((double)largest_value_with_single_unit_resolution) / log(2));
    ...
    cfg->sub_bucket_mask = ((int64_t)cfg->sub_bucket_count - 1)
                           << cfg->unit_magnitude;
    ...
    cfg->bucket_count = buckets_needed_to_cover_value(
...
void hdr_init_preallocated(struct hdr_histogram* h,
                           struct hdr_histogram_bucket_config* cfg)
{
             ...
             int significant_figures, struct hdr_histogram** result)
{
    ...
    int r = hdr_calculate_bucket_config(lowest_discernible_value,
                                        highest_trackable_value,
                                        significant_figures, &cfg);
    if (r) {
        return r;
    }
    ...
    histogram =
        (struct hdr_histogram*)hdr_calloc(1, sizeof(struct hdr_histogram));
    if (!histogram) {
        hdr_free(counts);
        return ENOMEM;
    }
...
int hdr_alloc(int64_t highest_trackable_value, int significant_figures,
...
bool hdr_record_values_atomic(struct hdr_histogram* h, int64_t value,
                              int64_t count)
{
    int32_t counts_index;
...
bool hdr_record_corrected_value(struct hdr_histogram* h, int64_t value,
                                int64_t expected_interval)
{
    return hdr_record_corrected_values(h, value, 1, expected_interval);
}

bool hdr_record_corrected_value_atomic(struct hdr_histogram* h, int64_t value,
                                       int64_t expected_interval)
{
    return hdr_record_corrected_values_atomic(h, value, 1, expected_interval);
}

bool hdr_record_corrected_values(struct hdr_histogram* h, int64_t value,
                                 int64_t count, int64_t expected_interval)
{
    ...
    for (; missing_value >= expected_interval;
...
bool hdr_record_corrected_values_atomic(struct hdr_histogram* h, int64_t value,
                                        int64_t count,
                                        int64_t expected_interval)
{
    int64_t missing_value;
...
int64_t
hdr_add_while_correcting_for_coordinated_omission(struct hdr_histogram* h,
                                                  struct hdr_histogram* from,
                                                  int64_t expected_interval)
{
    struct hdr_iter iter;
    int64_t dropped = 0;
...
int64_t hdr_value_at_percentile(const struct hdr_histogram* h,
                                double percentile)
{
    struct hdr_iter iter;
    int64_t total = 0;
...
int hdr_value_at_percentiles(const struct hdr_histogram* h,
                             const double* percentiles, int64_t* values,
                             size_t length)
{
    ...
    for (size_t i = 0; i < length; i++) {
        const double requested_percentile =
            percentiles[i] < 100.0 ? percentiles[i] : 100.0;
...
double hdr_stddev(const struct hdr_histogram* h)
{
    ...
    while (hdr_iter_next(&iter)) {
        if (0 != iter.count) {
            double dev =
                (hdr_median_equivalent_value(h, iter.value) * 1.0) - mean;
...
bool hdr_values_are_equivalent(const struct hdr_histogram* h, int64_t a,
                               int64_t b)
{
    return lowest_equivalent_value(h, a) == lowest_equivalent_value(h, b);
}

int64_t hdr_lowest_equivalent_value(const struct hdr_histogram* h,
                                    int64_t value)
{
    return lowest_equivalent_value(h, value);
}
...
static bool move_next(struct hdr_iter* iter)
{
    ...
    iter->highest_equivalent_value =
        highest_equivalent_value(iter->h, iter->value);
    iter->lowest_equivalent_value =
        lowest_equivalent_value(iter->h, iter->value);
    iter->median_equivalent_value =
        hdr_median_equivalent_value(iter->h, iter->value);
...
static void update_iterated_values(struct hdr_iter* iter,
...
bool hdr_iter_next(struct hdr_iter* iter) { return iter->_next_fp(iter); }
...
static bool percentile_iter_next(struct hdr_iter* iter)
{
    ...
    do {
        double current_percentile =
            (100.0 * (double)iter->cumulative_count) / iter->h->total_count;
        if (iter->count != 0 &&
            percentiles->percentile_to_iterate_to <= current_percentile) {
            update_iterated_values(
                iter, highest_equivalent_value(iter->h, iter->value));

            percentiles->percentile = percentiles->percentile_to_iterate_to;
            temp =
                (int64_t)(log(100 / (100.0 -
                                     (percentiles->percentile_to_iterate_to))) /
                          log(2)) +
                1;
            half_distance = (int64_t)pow(2, (double)temp);
            percentile_reporting_ticks =
                percentiles->ticks_per_half_distance * half_distance;
            percentiles->percentile_to_iterate_to +=
                100.0 / percentile_reporting_ticks;
...
void hdr_iter_percentile_init(struct hdr_iter* iter,
                              ...
                              int32_t ticks_per_half_distance)
{
    ...
    iter->specifics.percentiles.ticks_per_half_distance =
...
#pragma warning(disable : 4996)
    ...
    switch (format) {
    case CSV:
        snprintf(str, len, format_str, "%.", significant_figures,
                 "f,%f,%d,%.2f\r\n");
        break;
    case CLASSIC:
        snprintf(str, len, format_str, "%12.", significant_figures,
                 "f %12f %12d %12.2f\r\n");
        break;
    default:
        snprintf(str, len, format_str, "%12.", significant_figures,
...
static bool recorded_iter_next(struct hdr_iter* iter)
{
    while (basic_iter_next(iter)) {
        if (iter->count != 0) {
            ...
            iter->specifics.recorded.count_added_in_this_iteration_step =
                iter->count;
            return true;
        }
    }
...
void hdr_iter_recorded_init(struct hdr_iter* iter,
                            const struct hdr_histogram* h)
{
    hdr_iter_init(iter, h);
    ...
    if (has_next(iter) ||
        ...
        do {
            if (iter->value >=
                ...
                update_iterated_values(iter,
                                       linear->next_value_reporting_level);

                linear->next_value_reporting_level +=
                    linear->value_units_per_bucket;
                linear->next_value_reporting_level_lowest_equivalent =
                    lowest_equivalent_value(iter->h,
                                            linear->next_value_reporting_level);
...
void hdr_iter_linear_init(struct hdr_iter* iter, const struct hdr_histogram* h,
                          int64_t value_units_per_bucket)
{
    ...
    iter->specifics.linear.next_value_reporting_level_lowest_equivalent =
        lowest_equivalent_value(h, value_units_per_bucket);
    ...
    if (has_next(iter) ||
        ...
        do {
            if (iter->value >=
                ...
                logarithmic->next_value_reporting_level *=
                    (int64_t)logarithmic->log_base;
                logarithmic->next_value_reporting_level_lowest_equivalent =
                    lowest_equivalent_value(
                        iter->h, logarithmic->next_value_reporting_level);
...
void hdr_iter_log_init(struct hdr_iter* iter, const struct hdr_histogram* h,
                       int64_t value_units_first_bucket, double log_base)
{
    hdr_iter_init(iter, h);
    ...
    iter->specifics.log.next_value_reporting_level_lowest_equivalent =
        lowest_equivalent_value(h, value_units_first_bucket);
...
int hdr_percentiles_print(struct hdr_histogram* h,
                          int32_t ticks_per_half_distance, double value_scale,
                          format_type format)
{
    char line_format[25];
    ...
    printf(head_format, "Value", "Percentile", "TotalCount",
           "1/(1-Percentile)");
    ...
    while (hdr_iter_next(&iter)) {
        ...
        printf(line_format, value, percentile, total_count,
               inverted_percentile);
    }
    ...
    if (CLASSIC == format) {
        ...
        printf(CLASSIC_FOOTER, mean, stddev, max, h->total_count,
